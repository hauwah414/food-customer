<div class="mt-3">
    @if (count($data['data']) >= 1)
        @foreach ($data['data'] ?? [] as $key => $itemSearch)
            @if ($key >= 1)
                <hr>
            @endif
    ...
    <div class="pagination_ py-5">
        <nav>
            <ul class="pagination justify-content-center">
                <li class="page-item @if (!isset($data['prev_page_url'])) disabled @endif ">
                    <button type="button" class="page-link" id="paginationSearch"
                        data-page="{{ $data['current_page'] - 1 }}" aria-label="Previous">
                ...
                @if ($data['last_page'] >= 5)
                    @if ($data['last_page'] - $data['current_page'] >= 3)
                        <li class="page-item  active">
                            <button type="button" class="page-link" id="paginationSearch"
                                data-page="{{ $data['current_page'] }}">{{ $data['current_page'] }}</button>
                        </li>
                    @elseif ($data['last_page'] - $data['current_page'] <= 3)
                        <li class="page-item">
                ...
                @else
                    @for ($i = 1; $i <= $data['last_page']; $i++)
                        <li class="page-item @if ($data['current_page'] == $i) active @endif">
                            <button type="button" class="page-link" id="paginationSearch"
                                data-page="{{ $i }}">{{ $i }}</button>
                        </li>
                    @endfor
                @endif
                <li class="page-item @if (!isset($data['next_page_url'])) disabled @endif">
                    <button type="button" class="page-link" id="paginationSearch"
                        data-page="{{ $data['current_page'] + 1 }}" aria-label="Next">
